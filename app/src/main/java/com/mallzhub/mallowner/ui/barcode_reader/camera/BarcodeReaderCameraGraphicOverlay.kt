...
package com.mallzhub.mallowner.ui.barcode_reader.camera
...
import com.mallzhub.mallowner.ui.barcode_reader.BarcodeReaderUtils
 ...
 */
class BarcodeReaderCameraGraphicOverlay(context: Context, attrs: AttributeSet) : View(context, attrs) {
     ...
     * instances to the overlay using [BarcodeReaderCameraGraphicOverlay.add].
     */
    abstract class Graphic protected constructor(protected val overlay: BarcodeReaderCameraGraphicOverlay) {
     ...
     * coordinates later.
     */
    fun setCameraInfo(cameraSource: BarcodeReaderCameraSource) {
        val previewSize = cameraSource.previewSize ?: return
        if (BarcodeReaderUtils.isPortraitMode(context)) {
