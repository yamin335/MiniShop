...
import com.mallzhub.mallowner.ui.barcode_reader.BarcodeReaderPreferenceUtils
...
class BarcodeReaderCameraSource(private val graphicOverlay: BarcodeReaderCameraGraphicOverlay) {
    ...
    private var frameProcessor: BarcodeReaderCameraFrameProcessor? = null
    ...
    fun setFrameProcessor(processor: BarcodeReaderCameraFrameProcessor) {
        graphicOverlay.clear()
        synchronized(processorLock) {
            frameProcessor?.stop()
    ...
    private fun setPreviewAndPictureSize(camera: Camera, parameters: Parameters) {
        ...
        val sizePair: BarcodeReaderCameraSizePair = BarcodeReaderPreferenceUtils.getUserSpecifiedPreviewSize(context) ?: run {
            ...
            val displayAspectRatioInLandscape: Float =
                if (BarcodeReaderUtils.isPortraitMode(graphicOverlay.context)) {
        ...
        previewSize = sizePair.preview.also {
            ...
            BarcodeReaderPreferenceUtils.saveStringPreference(context, R.string.pref_key_rear_camera_preview_size, it.toString())
        }

        sizePair.picture?.let { pictureSize ->
            Log.v(TAG, "Camera picture size: $pictureSize")
            parameters.setPictureSize(pictureSize.width, pictureSize.height)
            BarcodeReaderPreferenceUtils.saveStringPreference(
    ...
    private inner class FrameProcessingRunnable internal constructor() : Runnable {
        ...
        override fun run() {
            ...
            while (true) {
                ...
                try {
                    synchronized(processorLock) {
                        val frameMetadata = BarcodeReaderCameraFrameMetadata(previewSize!!.width, previewSize!!.height, rotationDegrees)
                        data?.let {
    ...
    companion object {
         ...
         */
        private fun selectSizePair(camera: Camera, displayAspectRatioInLandscape: Float): BarcodeReaderCameraSizePair? {
            val validPreviewSizes = BarcodeReaderUtils.generateValidPreviewSizeList(camera)

            var selectedPair: BarcodeReaderCameraSizePair? = null
            ...
            for (sizePair in validPreviewSizes) {
                ...
                if (abs(aspectRatioDiff - minAspectRatioDiff) < BarcodeReaderUtils.ASPECT_RATIO_TOLERANCE) {
